...
@section('content-body')
    <form action="/employees" method="POST">
        ...
        <div class="row g-3 align-items-center">
            ...
            <div class="col-4">
                <input type="text" id="first_nm" name="first_nm" class="form-control" value="{{ $employee->first_nm }}">
            </div>
            <div class="col-2">
            ...
            <div class="col">
                <select name="company_id" id="company_id" class="form-control">
                  @foreach ($companies as $company)
                    <option value="{{ $company->id }}" {{ $employee->id == $company->id ? 'selected' : '' }}>{{ $company->name }}</option>
                  @endforeach
                </select>
            </div>
            {{-- <div class="col-auto">
